...
function TerminalPane({ tabId, isActive }: { tabId: string; isActive: boolean }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let retryCount = 0;
    const maxRetries = 100; // ~1.6 seconds at 60fps
    ...
    const initTerminal = () => {
      ...
      // Use double RAF to ensure container has final dimensions
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (cancelled) return;
          try {
            fitAddon.fit();
            // Force a resize event to ensure WebSocket gets correct dimensions
      ...
      if (fitAddonRef.current && xtermRef.current && terminalRef.current) {
        ...
        requestAnimationFrame(() => {
          try {
            ...
            // Update WebSocket with new dimensions
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#0a0a0a]" style={{ minHeight: 0 }}>
      ...
      <Card className={cn('flex-1 flex flex-col overflow-hidden', isFullscreen && 'rounded-none border-0')}>
        ...
        <div className="flex-1 overflow-hidden" style={{ minHeight: 0 }}>
          {tabs.map((tab) => (
            <div
              key={tab.id}
              className={cn('h-full w-full', tab.id === activeTab ? 'block' : 'hidden')}
              style={{ minHeight: 0 }}
            >
